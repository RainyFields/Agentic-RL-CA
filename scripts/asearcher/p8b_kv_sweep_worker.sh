...
HLOG=/mnt/hdfs/mlsys/users/xiaoxuan/agentic_rl_ca/logs/p8b_kv_sweep
...
for U in 0.5 0.60 0.65; do
  ...
  export ASYNC_DIAG_DIR="$DIAG_BASE/kvsweep_$TAG"
  mkdir -p "$ASYNC_DIAG_DIR"
  # 1Hz GPU sampler for this run (peak memory evidence)
  ( while true; do echo "$(date +%s) $(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits | tr '\n' ';')"; sleep 1; done \
      > "$ASYNC_DIAG_DIR/gpu_samples.txt" ) &
  SAMPLER_PID=$!
  export EXP_NAME="kvsweep_${TAG}_token_grpo_s0"
  GPU_MEMORY_UTIL="$U" TOTAL_STEPS=1 VAL_FREQ=1000000 SAVE_FREQ=1000000 \
