...
HLOG=/mnt/hdfs/mlsys/users/xiaoxuan/agentic_rl_ca/logs/p8b_prefill_sweep
...
echo "==== PREFILL-SWEEP start $(date -u) on $(hostname) ===="; nvidia-smi -L | head -2 || true
...
LOGDIR="$REPO/outputs"
for CFG in "B:32768:sticky" "C:17408:least_loaded" "D:32768:least_loaded"; do
  IFS=: read -r TAG BUDGET ROUTING <<< "$CFG"
  echo "==== PREFILL-SWEEP RUN $TAG budget=$BUDGET routing=$ROUTING $(date -u) ===="
  export ASYNC_DIAG_DIR="$DIAG_BASE/prefill_$TAG"
  ...
  export EXP_NAME="prefillsweep_${TAG}_token_grpo_s0"
  GPU_MEMORY_UTIL=0.65 MAX_NUM_BATCHED_TOKENS="$BUDGET" TOTAL_STEPS=4 VAL_FREQ=1000000 SAVE_FREQ=1000000 \
    ...
    timeout 2h bash "$REPO/scripts/run_condition.sh" token_grpo 0 "$PROTOCOL" \
      ...
      actor_rollout_ref.rollout.mode=async +env.async_rollout_enable=true +env.async_rollout_routing="$ROUTING"
  RC=$?
  kill "$SAMPLER_PID" ${DMON_PID:+$DMON_PID} 2>/dev/null || true
  echo "==== PREFILL-SWEEP RUN $TAG exit=$RC $(date -u) ===="
  ...
  if [ "$RC" != "0" ]; then echo "PREFILL-SWEEP ABORT: run $TAG failed (rc=$RC)"; exit 1; fi
  ...
  if dmesg 2>/dev/null | tail -50 | grep -qi "out of memory"; then
    echo "PREFILL-SWEEP ABORT: system OOM signature after run $TAG"; exit 1
  fi
done

status=DONE
echo "==== PREFILL-SWEEP complete $(date -u) ===="
