...
( cd "$REPO" && unset VIRTUAL_ENV PYTHONPATH PYTHONHOME \
  && export OMP_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8 MKL_NUM_THREADS=8 TOKENIZERS_PARALLELISM=false \
...
# ---- 5b. retriever watchdog: base2's retriever hung silently under the first rollout
# wave (served 200s then stopped answering; trainer retried 60s-timeouts forever).
# Health-check every 2min; after 3 consecutive failures, restart the server (index
# reload ~2min; the trainer client's retry/backoff rides through the gap).
serve_retriever() {
  ( cd "$REPO" && unset VIRTUAL_ENV PYTHONPATH PYTHONHOME \
    && export PATH="$RETR_ENV/bin:$PATH" LD_LIBRARY_PATH="$RETR_ENV/lib" OMP_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8 MKL_NUM_THREADS=8 TOKENIZERS_PARALLELISM=false \
...
export VAL_BATCH_OVERRIDE="${DIAG_VAL_BATCH:-1280}"   # env pool size == concurrent rollouts (1280 = P2-proven; 2048 hung base2 retriever)
