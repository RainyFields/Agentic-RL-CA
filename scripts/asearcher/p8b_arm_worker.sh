...
HLOG=/mnt/hdfs/mlsys/users/xiaoxuan/agentic_rl_ca/logs/p8b_arm_${COND}
...
finish() { echo "$status $(date -u +%FT%TZ)" > "$HLOG/$status"
  ...
  cp "$REPO/outputs/retriever/retriever_p8b_arm_${COND}.log" "$HLOG/" 2>/dev/null || true
...
echo "==== P8B-ARM $COND start $(date -u) on $(hostname) ===="; nvidia-smi -L | head -2 || true
...
RETR_LOG="$REPO/outputs/retriever/retriever_p8b_arm_${COND}.log"
serve_retriever() {
