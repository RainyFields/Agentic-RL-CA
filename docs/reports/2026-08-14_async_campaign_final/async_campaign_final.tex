...
estimator & mechanism & val@0 & val@25 & val@50 & val@75 & turns@75\\\midrule
turn-PPO (b0) & turn-level GAE + critic & 0.195 & 0.234 & 0.479 & \hw{0.561} & 7.5\\
GiGPO & anchor-state + episode groups & 0.195 & 0.064 & 0.438 & 0.535 & 16.1\\
HCAPO & hindsight answer-cond., $\gamma{=}0.95$ & 0.195 & 0.297 & 0.449 & 0.529 & \hw{3.7}\\
GRPO & token-level group-relative & 0.195 & 0.227 & \hw{0.520} & 0.520 & 17.0\\
token-PPO & stock token GAE + critic & 0.195 & \hw{0.312} & 0.447 & 0.426 & 8.4\\\bottomrule
\end{tabular}
\caption{Estimator comparison on the lockstep engine (\texttt{fast75}), all arms complete.
token-PPO / HCAPO / GiGPO died with their pods at steps 42/44/46, were resumed 2026-08-14
from step 25 on identical code (restart re-vals reproduced 0.312/0.297/0.064 exactly), and
finished 2026-08-15/16.}
\end{table}
\begin{figure}[h]\centering
\includegraphics[width=\linewidth]{assets/fig_estimators.png}
\caption{Five-estimator training curves, all complete. turn-PPO wins (0.561); GiGPO (0.535)
and HCAPO (0.529) edge past GRPO (0.520); token-PPO trails (0.426). \hw{GiGPO's step-25 val
collapse (0.064) fully recovered} --- its train success was normal ($\sim$0.45) throughout;
the dip was a transient greedy-decode pathology (17.8 val turns at step 25), not estimator
failure. \hw{token-PPO regressed 0.447$\rightarrow$0.426 over steps 50--75} --- both late
regressions in the campaign (this and async turn-PPO, \S 5) are critic-based arms. HCAPO is
the turn-efficiency standout: 0.529 at 3.7 turns/question, half of turn-PPO's 7.5; its
$\gamma{=}0.95$ is a deliberate protocol deviation (paper value). val@25 was weakly
predictive of val@75: the step-25 leader finished last, the step-25 collapse finished
second.}
\end{figure}

\paragraph{HCAPO: incipient per-turn length runaway.} HCAPO is the only arm that never
compresses per-turn responses (median 270--400 tok/turn throughout, vs.\ $\sim$30 for
GRPO/GiGPO and $\sim$100--120 for the PPOs; p90 pinned at the 1{,}024 response cap for the
entire run). Over steps 62--75 the median climbs $\sim$280$\rightarrow$390 with spikes to
716 (step 72) and \hw{1{,}024 --- the cap --- at step 75}, while the valid-action ratio
sags 0.84$\rightarrow$0.79 (cap truncation). This is mechanism-consistent:
$\gamma{=}0.95$ discounts per \emph{turn}, so content migrates into fewer, longer turns,
and nothing in the hindsight objective penalizes within-turn tokens. Trajectory length is
\emph{not} running away (turns fell 4.8$\rightarrow$3.4) and reward was unaffected through
step 75, but longer training would likely saturate the cap; follow-ups should add a
per-token length penalty or a per-turn cap margin.
\begin{figure}[h]\centering
\includegraphics[width=0.85\linewidth]{assets/fig_resp_len.png}
\caption{Per-turn response length over training (batch median, EMA; faint = raw). Four
arms compress within $\sim$20 steps (GRPO/GiGPO to $\sim$30 tok/turn, the PPOs to
$\sim$100--120); HCAPO alone plateaus at 250--300 and turns upward late, with the raw
median hitting the 1{,}024 cap at step 75. Shaded band: HCAPO p10--p90 --- the upper edge
sits at the cap for the entire run.}
\end{figure}
