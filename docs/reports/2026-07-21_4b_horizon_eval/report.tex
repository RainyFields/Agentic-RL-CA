...
over-search on shallow comparison questions. A thinking-mode ablation confirms the SearchQA
ordering is not a truncation artifact: with \texttt{enable\_thinking=False} at the same budget both
arms match or beat their thinking counterparts using 66--77\% fewer tokens, and GRPO still leads
turn-PPO. We release all figures, the rerunnable analysis, and the recovered required-hops
annotations.
\end{abstract}
...
\subsection*{Robustness: does the SearchQA ranking survive without thinking mode?}
The SearchQA results above use Qwen3's thinking mode with a 2048-token response budget, under which
the critic arm clips 8.4\% of its turns --- a clipped turn emits no action and so wastes one of only
four. That makes truncation a live confound for the report's central SearchQA claim (flat GRPO
$>$ critic turn-PPO): the gap might be a truncation artifact rather than a credit-assignment one.
We therefore re-ran both arms with \texttt{enable\_thinking=False} at the \emph{same} 2048 budget,
500 steps, batch, seeds and $\gamma{=}\lambda{=}1$ --- so \emph{only} the thinking flag differs.

\begin{table}[h]\centering\small
\caption{Thinking-mode ablation, SearchQA 4B --- \textbf{full-set macro-EM} (51{,}713 q, step-500
checkpoints; same metric as Table 1). Non-thinking beats thinking for \emph{both} methods at
$\approx$4$\times$ fewer tokens per turn, and the GRPO-over-turn-PPO ordering \textbf{survives}
(gap $0.052\to0.046$). GRPO non-thinking (0.463) is the best SearchQA result in this report.}
\begin{tabular}{lccc c cc}
\toprule
& \multicolumn{3}{c}{\textbf{macro-EM}} && \multicolumn{2}{c}{\textbf{mean resp.\ tokens/turn}}\\
\cmidrule(lr){2-4}\cmidrule(lr){6-7}
& thinking & non-thinking & $\Delta$ && thinking & non-thinking\\
\midrule
GRPO      & 0.441 & \good{\textbf{0.463}} & $+0.022$ && 476 & \good{111}\\
turn-PPO  & 0.389 & \good{0.417} & $+0.028$ && 610 & \good{207}\\
\midrule
GRPO $-$ turn-PPO & $0.052$ & $0.046$ & & & &\\
\bottomrule
\end{tabular}
\end{table}

Three readings. (i) \textbf{The finding is robust}: GRPO still leads turn-PPO when the truncation
channel is largely removed (turn-PPO's clip falls 0.084 $\to$ 0.057), so the ordering is not a
clipping artifact; the gap barely moves ($0.052\to0.046$). (ii) \textbf{Thinking is not paying for itself here}: both arms do at least as
well without it while generating 66--77\% fewer tokens, i.e.\ the 4-turn budget rewards deciding
\emph{what to search for} far more than deliberating about it. (iii) \textbf{The critic arm gains
most} ($+0.033$ vs $+0.009$), consistent with it having been the arm penalised by truncation.
\emph{Caveat:} single seed per cell, and GRPO's $+0.009$ sits inside that run's own step-to-step
oscillation band ($0.429$--$0.457$ over steps 250--400); the defensible claim is that non-thinking
is \emph{not worse and much cheaper}, not that it is better.
