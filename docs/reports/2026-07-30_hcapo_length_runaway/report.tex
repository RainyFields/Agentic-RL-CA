...
\textbf{Running now:} an offline diagnostic that re-scores logged rollouts under matched conditions
(with and without the hindsight text, and with the \emph{gold answer} injected instead) to measure
exactly how much of HCAPO's score is length/position nuisance versus genuine hindsight
information --- including whether the ``pivotal turn'' it picks even survives the controls.
...
\item \textbf{Exclude the final (answer) turn} from hindsight credit --- the ending trivially
``explains'' the answer turn, a leak no normalisation removes.
