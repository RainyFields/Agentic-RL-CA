...
and first evidence that the paper-correct form does not}
...
$1.0$, two-sided (the upper clip binds for the first time), with growing rather than collapsing
spread. Forty-five steps into the corrected run, length and clipping are normal; the legacy
runaway's onset window (steps $\sim$75--200) has not yet been reached, so the behavioural
confirmation is preliminary. All prior HCAPO numbers from this codebase (SearchQA 1.7B/4B, AlfWorld
1.7B) characterise the buggy variant, not the published method.
\end{abstract}
...
same path $\sim$125 steps later; the paper-correct run (green, 45 steps so far) is flat --- though it
has not yet reached the window where the legacy runs took off.}
\label{fig:runaway}
\end{figure}
...
\section{Initial observations: why the corrected form does not explode}

\textbf{Structural reason.} The ratchet needs a way for a rollout to raise its $\rho$ by being
globally more verbose than its siblings. Under Eq.~7 it has none: $\rho_t$ compares turn $t$ only
against \emph{the same trajectory's other turns}, and numerator and denominator carry the same
$1/|a|$ normalisation, so uniform verbosity cancels identically. What remains is a bounded,
\emph{relative} preference between turns of one trajectory (a longer turn can score somewhat above
its shorter siblings), clipped to $[0.8,1.2]$ and non-compounding --- there is no cross-rollout
length gradient for group normalisation to amplify.

\textbf{Empirical, first 45 steps} (single seed; all values from the run's per-step
instrumentation):
\begin{center}\small
\begin{tabular}{lcc}
\toprule
...
(green) sits at $1.0$ from step one. \emph{Right:} legacy spread collapses as $\rho$ saturates; the
corrected spread grows --- the hindsight signal discriminates between turns rather than encoding
response length.}
\label{fig:rho}
\end{figure}

\textbf{What this does not yet show.} The legacy runaway ignited in the window steps
$\sim$75--200 ($171\to487\to1371$ tokens). The corrected run has not reached it; ``no explosion''
is therefore a structural argument plus 45 clean steps, not yet a completed demonstration. The
decisive within-batch test --- per-sample $\mathrm{corr}(\rho, n_{tok})$, predicted $\gg0$ for the
legacy form and $\approx0$ for Eq.~7 --- is queued for the next instrumentation pass. This note will
be updated at steps 100/200/500.
